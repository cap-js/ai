...
export default function registerHandlersForEmbeddingSearch(srv) {
	srv.before('READ', async (req) => {
		if (
			...
			!req.target['@ai.embeddingSearch']?.length ||
			req.target.isDraft
		)
			return;
		...
		if (cqn.SELECT.where.at(-1).SELECT.where[0].args[0].list) {
			...
			const ele = cqn.SELECT.where.at(-1).SELECT.where[0].args[0];
			if (ele.ref[0].startsWith('$')) {
				ele.ref.shift();
			}
		}
		...
		req.query.SELECT.where = req.query.SELECT.where?.length
			? [{ xpr: req.query.SELECT.where }, 'and', { xpr: where }]
			: where;
	});
}
				...
				args: [
					{ ref: [vector] },
					{
						...
						args: [
							{ val: searchTerm },
							{ val: 'QUERY' },
							{ val: entity.elements[vector]?.['@ai.model'] ?? cds.env.ai.embeddings.defaultModel }
						]
					}
				]
			},
