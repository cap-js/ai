...
export default function registerHandlersForRecommendations(srv) {
	srv.prepend(() =>
		srv.on('READ', async (req, next) => {
			...
			if (!req.target.isDraft || !req.target.actives['@UI.Recommendations'] || !req.query.elements[req.target['@UI.Recommendations']['=']] || req._?.event === 'draftActivate') {
				return next();
			}

			const model = cds.context.model ?? cds.model;
			const recommendationsStruct = req.target.actives['@UI.Recommendations']['='];
			const recommendationsDefinition = model.definitions[req.target.actives.elements[recommendationsStruct].target];

			const columns = Object.keys(req.target.actives.elements).filter(
				(ele) => ele !== 'modifiedAt' && ele !== 'modifiedBy' && ele !== 'createdAt' && ele !== 'createdBy' && req.target.actives.elements[ele].type !== 'cds.LargeBinary' && req.target.actives.elements[ele].type !== 'cds.Vector'
			);

			const recommendationsIdx = req.query.SELECT.columns.findIndex((col) => col.ref && col.ref[0] === req.target['@UI.Recommendations']['=']);
			...
			const dynamicRecommendations = Object.keys(fieldsWithRecommendations).reduce((acc, field) => {
				if (req.target.elements[field]['@UI.RecommendationState']?.xpr) {
					acc.push(Object.assign({ as: `SAP_RECOMMENDATION_STATE_${field}` }, req.target.elements[field]['@UI.RecommendationState']));
				}
				return acc;
			}, []);
			...
			const [records, resWithAllColumns, res] = await Promise.all([SELECT.from(req.target.actives).columns(columns).where(where).limit(2018), SELECT.from(req.subject).columns(columns.concat(dynamicRecommendations)), next()]);
			...
			for (const row of response) {
				const propsToDelete = Object.keys(row).filter((prop) => prop.startsWith(req.target['@UI.Recommendations']['=']));
				...
				for (const ele in fieldsWithRecommendations) {
					if (predictionRow[`SAP_RECOMMENDATION_STATE_${ele}`] === undefined || predictionRow[`SAP_RECOMMENDATION_STATE_${ele}`] != 0) {
						predictionRow[ele] = '[PREDICT]';
					} else {
						fieldsWithDisabledRecommendations[ele] = 1;
					}
				}
				for (const ele of Object.keys(predictionRow).filter((ele) => ele.startsWith(`SAP_RECOMMENDATION_STATE_`))) {
					delete predictionRow[ele];
				}
				contextRows.push(predictionRow);
			}

			const { predictions, details } = await aiCore.fetchPredictions({ rows: contextRows, predictionColumns: Object.keys(fieldsWithRecommendations) });
			LOG.debug(details);
						...
						row[recommendationsStruct][ele] = prediction[ele]
							...
							.map((entry, idx) => {
								return {
									...
									RecommendedFieldDescription: descriptions.find((descr) => descr[ele] == entry.prediction)?.[`SAP_Recommendations_Descr_${ele}`] ?? '',
...
const matchRow = (row1, row2) => (row1.ID !== undefined && row1.ID == row2.ID) || (row1.SAP_RECOMMENDATIONS_ID !== undefined && row1.SAP_RECOMMENDATIONS_ID == row2.SAP_RECOMMENDATIONS_ID);
