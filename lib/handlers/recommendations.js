	...
	srv.prepend(() =>
		srv.on('READ', async (req, next) => {
				...
				if (req.target.elements[field]['@UI.RecommendationState']?.xpr) {
					acc.push(
						Object.assign(
							...
							req.target.elements[field]['@UI.RecommendationState']
						)
					);
				}
				return acc;
			...
			const [records, resWithAllColumns, res] = await Promise.all([
				// The 2018 limit is based on https://help.sap.com/docs/sap-ai-core/generative-ai/sap-rpt-1?locale=en-US; we use the small model, e.g. 2048 max rows; used a smaller value for buffer
				SELECT.from(req.target.actives).columns(columns).where(where).limit(2018),
