...
export default function registerHandlersForRecommendations(srv) {
  srv.prepend(() =>
    srv.on('READ', async (req, next) => {
      // Defense in depth: cds-plugin.js already filters out services with
      // `srv.mocked = true`, but a free-form READ on any other service
      // (e.g. via `srv.send(new cds.Request({ method, path }))`) can leave
      // req.target undefined. Bail out before dereferencing rather than
      // throwing TypeError.
      ...
      const columns = Object.keys(req.target.actives.elements).filter(
        (ele) =>
          ...
          ele !== 'createdAt' &&
          ele !== 'createdBy' &&
          // Associations/compositions (incl. the draft-added DraftAdministrativeData)
          // have no RPT-1 dtype and would leak non-scalar values into /predict rows,
          // which RPT-1 rejects with HTTP 422. Their flattened foreign keys (e.g.
          // author_ID) are separate scalar elements and remain included.
