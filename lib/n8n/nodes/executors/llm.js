...
export async function callLlm(cds, messages, { model, service } = {}) {
  ...
  // Primary: configured llm service via cds.connect.to()
  try {
    const serviceName = service ?? 'llm'
    const svc = await cds.connect.to(serviceName)
    const result = await svc.send('chat', { messages: lcMessages, ...(model ? { model } : {}) })
    return extractContent(result)
  } catch (e) {
    // fall through to AICore
    cds.log('n8n:llm').warn('llm service unavailable, falling back to AICore:', e.message)
  }
  ...
  try {
    ...
    cds.log('n8n:llm').warn('AICore fallback failed:', e.message)
