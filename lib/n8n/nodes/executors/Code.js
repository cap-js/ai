...
export async function execute(node, input, context) {
  ...
  if (code.includes('require(') || code.includes('import ')) {
    cds.log('n8n:code').warn(`[n8n Code node "${node.name}"] Code contains require() or import — these are not supported in the CAP-native executor. Use ES module imports in the workflow definition instead.`)
  }
