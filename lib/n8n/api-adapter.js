...
export function buildN8nRouter(cfg = {}) {
  ...
  router.get('/rest/push', (req, res) => {
    ...
    _sseClients.add(res)
    // SSE keep-alive: cleared on connection close — not business logic, Express transport layer
  ...
  router.post('/api/v1/workflows', async (req, res) => {
    try {
      ...
      await cds.tx({ user: req.user }, async tx => {
        await tx.run(INSERT.into(WF_ENTITY).entries(row))
      })
  ...
  router.put('/api/v1/workflows/:id', async (req, res) => {
    try {
      ...
      await cds.tx({ user: req.user }, async tx => {
        await tx.run(UPDATE(WF_ENTITY).set(update).where({ ID: req.params.id }))
      })
      const row = await db.run(SELECT.one.from(WF_ENTITY).where({ ID: req.params.id }))
      if (!row) return res.status(404).json({ message: 'Not found' })
      res.json({ data: rowToWorkflow(row) })
  ...
  router.delete('/api/v1/workflows/:id', async (req, res) => {
    try {
      await cds.tx({ user: req.user }, async tx => {
        await tx.run(DELETE.from(WF_ENTITY).where({ ID: req.params.id }))
      })
      res.json({})
    } catch (err) {
  ...
  router.post('/api/v1/workflows/:id/activate', async (req, res) => {
    try {
      const db = await cds.connect.to('db')
      await cds.tx({ user: req.user }, async tx => {
        await tx.run(UPDATE(WF_ENTITY).set({ active: true }).where({ ID: req.params.id }))
      })
      const row = await db.run(SELECT.one.from(WF_ENTITY).where({ ID: req.params.id }))
      if (!row) return res.status(404).json({ message: 'Not found' })
      res.json({ data: rowToWorkflow(row) })
  ...
  router.post('/api/v1/workflows/:id/deactivate', async (req, res) => {
    try {
      const db = await cds.connect.to('db')
      await cds.tx({ user: req.user }, async tx => {
        await tx.run(UPDATE(WF_ENTITY).set({ active: false }).where({ ID: req.params.id }))
      })
      const row = await db.run(SELECT.one.from(WF_ENTITY).where({ ID: req.params.id }))
      if (!row) return res.status(404).json({ message: 'Not found' })
      res.json({ data: rowToWorkflow(row) })
  ...
  router.delete('/api/v1/executions/:id', async (req, res) => {
    try {
      await cds.tx({ user: req.user }, async tx => {
        await tx.run(DELETE.from(EX_ENTITY).where({ ID: req.params.id }))
      })
      res.json({})
    } catch (err) {
    ...
    try {
      ...
      if (body.id) {
        ...
        if (existing) {
          ...
          await cds.tx({ user: req.user }, async tx => {
            await tx.run(UPDATE(WF_ENTITY).set({
  ...
  router.patch('/rest/workflows/:id', async (req, res) => {
    try {
      ...
      await cds.tx({ user: req.user }, async tx => {
        await tx.run(UPDATE(WF_ENTITY).set(updateFields).where({ ID: req.params.id }))
      })
      const row = await db.run(SELECT.one.from(WF_ENTITY).where({ ID: req.params.id }))
      res.json({ data: rowToWorkflow(row) })
    } catch (err) {
  ...
  router.get('/rest/workflow-history/workflow/:workflowId/version/:versionId', (_req, res) => {
    res.json({ data: null })
  })

  // ── /rest/workflows/:id/executions/last-successful ──────────────────────────
  router.get('/rest/workflows/:id/executions/last-successful', (_req, res) => {
    res.json({ data: null })
  })

  // ── /rest/workflow-dependencies/counts ──────────────────────────────────────
  router.post('/rest/workflow-dependencies/counts', (_req, res) => {
    res.json({ data: {} })
  })
