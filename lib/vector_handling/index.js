export default async function addSQLiteVectorSupport(dbc) {
  ...
  try {
    ...
    console.warn('Failed to load embedding module:', err.message);
    return;
  }
  ...
  try {
    ...
    console.warn('Failed to initialize embedding model, VECTOR_EMBEDDING will not be available:', err.message);
    return; // Don't register the function if embedding model fails
  }
