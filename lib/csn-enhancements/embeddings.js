...
export default function enhanceModel(model) {
	...
	// 	addSimilarEntitiesAssociation(q, entity, model)
	// }
	// 		continue;
	// 	}
	// }
...
const _sql = cds.compiler.to.sql;
cds.compiler.to.sql = function (csn, options) {
	if (options.sqlDialect !== 'sqlite') {
		return _sql(csn, options);
	}
	// For SQLite rewrite HANA functions for cosine similarity and l2distance
	const res = _sql(csn, options);
	for (let i = 0; i < res.length; i++) {
		const sql = res[i];
		res[i] = sql.replaceAll(`cosine_similarity(`, 'vec_distance_cosine(').replaceAll(`l2distance(`, 'vec_distance_L2(').replaceAll(`COSINE_SIMILARITY(`, 'vec_distance_cosine(').replaceAll(`L2DISTANCE(`, 'vec_distance_L2(');
	}
	return res;
};
Object.assign(cds.compiler.to.sql, _sql);
