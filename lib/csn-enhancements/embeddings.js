			...
			for (const ele in entity.elements) {
				...
				if (aiAnnotations.length) {
					...
					if (aiAnnotations.some((k) => !source.elements[alias ?? ele]?.[k])) {
						LOG.error(
							`@ai.embedding annotation annotated to view ${name}! Embedding annotations must be on fields of a non projected or selected entity!`
						);
					}
				}
			}
				...
				entity.elements[ele + '_embedding'] = {
					...
					value: {
						...
						args: [
							{ ref: [ele] },
							{ val: 'DOCUMENT' },
							{ val: element['@ai.embedding.@ai.model'] ?? cds.env.ai.embeddings.defaultModel }
						]
					}
				};
				if (element['@Search.fuzzinessThreshold']) {
					entity.elements[ele + '_embedding']['@Search.fuzzinessThreshold'] =
						element['@Search.fuzzinessThreshold'];
				}
		...
		if (entity.projection || entity.query) {
			...
			for (const ele in entity.elements) {
				...
				if (
					entity.elements[ele] &&
					col.type === 'cds.Vector' &&
					(!q.excluding || !q.excluding.some((exC) => exC === ele))
				) {
...
cds.compiler.to.hdi.migration = function (csn, options, beforeImage) {
	...
	for (const name in csn.definitions) {
		...
		for (const ele in entity.elements) {
			...
			if (
				element.value?.func === 'VECTOR_EMBEDDING' &&
				!element.value?.args?.[2]?.val?.startsWith('SAP') &&
				element.value?.args?.length === 3
			) {
	...
	for (const def of res.definitions) {
		if (entitiesWithRemoteSrv[def.name]) {
			def.sql = def.sql.replaceAll(
				`${cds.env.ai.embeddings.remoteSource}()`,
				cds.env.ai.embeddings.remoteSource
			);
		}
	}
