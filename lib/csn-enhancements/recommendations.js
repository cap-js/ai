...
function enhanceEntity(name, model) {
	...
	const vhFields = Object.keys(entity.elements).reduce((vhFields, ele) => {
		...
		if (entity.elements[ele]['@UI.RecommendationState'] !== 0 && (entity.elements[ele]['@Common.ValueList.CollectionPath'] || model.definitions[entity.elements[ele].target]?.['@cds.odata.valuelist'])) {
			if (entity.elements[ele].keys) {
				for (const key of entity.elements[ele].keys) {
					vhFields[ele + '_' + key.ref.join('_')] = structuredClone(model.definitions[entity.elements[ele].target].elements[key.ref]);
