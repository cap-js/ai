const MINILM_MODEL = Object.freeze({
...
const MODEL_PRESETS = new Map([[MINILM_MODEL.repository, MINILM_MODEL]]);
...
function resolveModelPreset(name) {
  ...
  if (!model) {
    throw new Error(
      `Unsupported embedding model '${name}' for ad-hoc download. Provision a compatible custom model with --descriptor and --directory.`
    );
  }
  return model;
}

export { MINILM_MODEL, findModelPreset, resolveModelPreset };
