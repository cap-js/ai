...
async function installModel(repository, options = {}) {
  ...
  while (true) {
    try {
      ...
      // A configured prompt override (embedding.prompts) takes precedence over the discovered
      // prompts at runtime; the persisted lock keeps the discovered prompts, so we override
      // only the returned model here.
      return {
        model: options.prompts ? { ...model, prompts: options.prompts } : model,
        modelDir,
        modelRoot
      };
    } catch (error) {
