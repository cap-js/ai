...
function createHuggingFaceClient(options = {}) {
  const { fetchImpl = globalThis.fetch, hubApi } = options;
  ...
  const loadedHubApi = resolveHubApi(hubApi);

  return {
    async getModelInfo(repository) {
      const { modelInfo } = await loadedHubApi;
    ...
    async getFiles(repository, revision) {
      const { listFiles } = await loadedHubApi;
      return runHubOperation(
    ...
    async getFile(repository, revision, remotePath) {
      const { downloadFile } = await loadedHubApi;
      return runHubOperation(
...
async function resolveHubApi(hubApi) {
  if (
    typeof hubApi?.modelInfo === 'function' &&
    typeof hubApi?.listFiles === 'function' &&
    typeof hubApi?.downloadFile === 'function'
  ) {
    return hubApi;
  }
  return { ...(await loadHuggingFaceHub()), ...hubApi };
}
