...
function getDataDir(appName = 'semantic-search', environment = process.env) {
  const home = os.homedir();
  const directory =
    os.platform() === 'win32'
      ? environment.LOCALAPPDATA || environment.APPDATA || path.join(home, 'AppData', 'Local')
      : environment.XDG_DATA_HOME || path.join(home, '.local', 'share');

  return path.join(directory, appName);
}

function getModelCacheRoot(environment = process.env) {
  return (
    environment.CDS_AI_MODEL_CACHE ||
    path.join(getDataDir('semantic-search', environment), 'models')
  );
}
...
async function provisionModel(model, options = {}) {
  ...
  const requestedDirectory = path.resolve(
    options.directory ?? getModelCacheDir(getModelCacheRoot(options.env), model)
  );
...
async function acquireInstallLock(lockPath, directory, owner) {
  try {
    ...
    throw new Error(`Embedding model directory ${directory} is already being provisioned`, {
      cause: error
    });
  }
}
...
async function loadModelAndTokenizer(modelDir, model) {
  const [{ Tokenizer }, { InferenceSession }] = await Promise.all([
    import('@huggingface/tokenizers'),
    import('./InferenceSession.js')
  ]);
