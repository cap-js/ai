...
function modelDescriptorDigest(model) {
  ...
  const canonical = JSON.stringify({
    ...
    ...(model.prompts
      ? {
          prompts: {
            ...(model.prompts.query ? { query: model.prompts.query } : {}),
            ...(model.prompts.document ? { document: model.prompts.document } : {})
          }
        }
      : {})
  });
...
async function loadTokenizerPackage(importModule = (specifier) => import(specifier)) {
  try {
    ...
    if (
      ...
      throw new Error(
        "Using ai-sqlite embeddings requires @huggingface/tokenizers@0.1.3. Install it with 'npm add @huggingface/tokenizers@0.1.3'.",
        { cause: error }
      );
    }
    throw error;
