...
// Map HANA's text_type argument onto the model's discovered Sentence Transformers prompts.
// Prompt-less models (no `model.prompts`) always resolve to '' and stay byte-identical.
function promptFor(prompts, textType) {
  if (!prompts) return '';
  const key = typeof textType === 'string' ? textType.toLowerCase() : '';
  if (key === 'query') return prompts.query ?? '';
  if (key === 'document') return prompts.document ?? '';
  return '';
}
...
async function createEmbeddingRuntimeFromModel(modelDir, model) {
  ...
  try {
    ...
    const runtime = {
      ...
      vectorEmbedding(text, textType) {
        // Treat empty and whitespace-only input as "no value": embedding blank text wastes an
        // inference and yields a semantically meaningless vector, so return the zero vector.
        if (!text || !String(text).trim())
          return JSON.stringify(new Array(model.dimensions).fill(0));
        const prefix = promptFor(model.prompts, textType);
        return JSON.stringify(Array.from(this.embedding(prefix ? `${prefix}${text}` : text)));
      },
      dispose
    };
...
async function resolveEmbeddingModel(configuration, options = {}) {
  ...
  const { model: modelName, directory, prompts } = normalizeEmbeddingConfiguration(configuration);
  ...
  try {
    ...
    return { ...installed, model: applyPromptConfiguration(installed.model, prompts) };
  }
  ...
  return { model: applyPromptConfiguration(model, prompts), modelDir };
}

function applyPromptConfiguration(model, configuredPrompts) {
  if (!configuredPrompts) return model;
  if (!model.output.includePrompt) {
    throw new Error(
      'embedding.prompts cannot be used because the model excludes prompt tokens from pooling'
    );
  }
  return { ...model, prompts: { ...model.prompts, ...configuredPrompts } };
}

async function installModelOnDemand(modelName, modelDir, options, warn) {
  warn(
    `Embedding model '${modelName}' is not available in '${modelDir}'. Downloading it now; application startup may be delayed. Only use models from repositories you trust. ${modelInstallHint(modelName)}`
  );
  try {
...
function normalizeEmbeddingConfiguration(configuration) {
  ...
  const model = configuration.model === undefined ? DEFAULT_EMBEDDING_MODEL : configuration.model;
  ...
  return {
    ...
    prompts: promptsFromConfig(configuration.prompts)
  };
}

// Turn the user-facing `embedding.prompts.{query,document}` config into a runtime prompts
// object. When set, it takes precedence over the discovered prompts: the returned prompts
// override whatever the lock carries, letting users supply prefixes for models whose prompts
// are not discoverable (e.g. nomic's README-only `search_query: ` / `search_document: `).
// Returns undefined when no prompts are configured, leaving discovered prompts in place.
function promptsFromConfig(configured) {
  if (configured === undefined) return undefined;
  if (typeof configured !== 'object' || configured === null || Array.isArray(configured)) {
    throw new TypeError('embedding.prompts must be an object with query and/or document strings');
  }

  const prompts = {};
  for (const key of ['query', 'document']) {
    if (configured[key] === undefined) continue;
    if (typeof configured[key] !== 'string' || !configured[key]) {
      throw new TypeError(`embedding.prompts.${key} must be a non-empty string`);
    }
    prompts[key] = configured[key];
  }
...
function processEmbedding(input, session, model) {
  const results = session.run(createFeeds(input, session.inputNames));
...
function createFeeds(encoding, inputNames = STANDARD_INPUT_NAMES) {
  const {
    ids,
  ...
  const supportedInputs = new Set(inputNames);
  return Object.fromEntries(
    Object.entries(values)
      .filter(([name]) => supportedInputs.has(name))
      .map(([name, data]) => [name, { type: 'int64', data, dims: dimensions }])
  );
}
