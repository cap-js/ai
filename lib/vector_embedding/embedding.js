...
async function resolveEmbeddingModel(configuration, options = {}) {
  ...
  if (configuration?.directory) {
    const modelDir = path.resolve(root, configuration.directory);
    if (typeof configuration.model !== 'string' || !configuration.model) {
      throw new Error('embedding.model is required when embedding.directory is configured');
    }
    ...
    try {
      ...
      const recovery = /Embedding model lock not found/.test(error.message)
        ? modelInstallHint(configuration.model, configuration.directory)
    ...
    const preset = findModelPreset(configuration.model);
    if (preset && modelDescriptorDigest(model) !== modelDescriptorDigest(preset)) {
      throw new Error(
        `Embedding model directory ${modelDir} does not contain the pinned ${configuration.model} preset. Choose another directory or remove it explicitly before provisioning it again.`
      );
    }
    if (!preset && model.repository !== configuration.model) {
      throw new Error(
        `Embedding model directory ${modelDir} contains ${model.repository}, not ${configuration.model}. Choose another directory or remove it explicitly before provisioning it again.`
      );
    }
    try {
      ...
      throw new Error(
        `${error.message}. ${modelInstallHint(configuration.model, configuration.directory)}`,
        {
          cause: error
        }
      );
  ...
  const model = resolveConfiguredModel(configuration);
  const modelDir = getModelCacheDir(getModelCacheRoot(env), model);
  try {
    await verifyModelDirectory(modelDir, model);
  } catch (error) {
    throw new Error(`${error.message}. ${modelInstallHint(model.repository)}`, {
      cause: error
    });
  }
  return { model, modelDir };
...
function resolveConfiguredModel(configuration) {
  if (configuration == null) return DEFAULT_MODEL;
  if (typeof configuration === 'string') return resolveModelPreset(configuration);
  if (configuration.model) return resolveModelPreset(configuration.model);
  return validateModelDescriptor(configuration);
}
