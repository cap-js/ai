...
function interpretPrompts(config, repository) {
  ...
  if (typeof config !== 'object' || Array.isArray(config)) {
    throw new Error(`Invalid Sentence Transformers configuration in '${repository}/${CONFIG_ST_FILE}'`);
  }
  ...
  if (typeof prompts !== 'object' || Array.isArray(prompts)) {
    throw new Error(`Unsupported Sentence Transformers prompts in '${repository}': expected an object`);
  }
