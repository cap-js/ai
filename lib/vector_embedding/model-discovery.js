...
async function discoverModelMetadata(repository, options) {
  ...
  const candidate = validateModelCandidate({
    ...
    output: {
      ...
      includePrompt: semantics.includePrompt
    },
...
async function readSentenceTransformerSemantics(context, repository, revision, filesByPath) {
  ...
  // modules.json defines the ordered execution pipeline. Accept only stages that this runtime
  // actually executes; skipping a custom or future module would silently change the embeddings.
  for (const module of modules) {
    if (!module || typeof module.type !== 'string')
      throw new Error(`Invalid Sentence Transformers module in '${repository}/${MODULES_FILE}'`);
    if (!PIPELINE_MODULES.has(module.type)) {
      throw new Error(
  ...
  const poolingPath = moduleConfigPath(modules[1], repository);
  ...
  if (
    poolingConfig.include_prompt !== undefined &&
    typeof poolingConfig.include_prompt !== 'boolean'
  ) {
    throw new Error(
      `Invalid include_prompt in Sentence Transformers pooling configuration for '${repository}'`
    );
  }
  const includePrompt = poolingConfig.include_prompt !== false;
  ...
  if (filesByPath.has(CONFIG_ST_FILE)) {
    ...
    // Intentionally checked post-normalization: empty-string prompts produce no prefix tokens,
    // so include_prompt=false is a no-op for those and should not cause rejection.
    if (prompts && !includePrompt) {
      throw new Error(
  ...
  if (modules[0]?.path) {
    sentenceConfigPaths.unshift(`${normalizedModulePath(modules[0].path)}/${SENTENCE_CONFIG_FILE}`);
  }
...
// Map the conventional retrieval prompts onto HANA text types. Sentence Transformers permits
// arbitrary additional prompt names, which do not affect QUERY/DOCUMENT and are ignored here.
function interpretPrompts(config, repository) {
  ...
  if (prompts.document) normalized.document = prompts.document;
