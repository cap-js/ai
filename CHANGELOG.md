...
- **Beta:** Add the `ai-sqlite` and `ai-sqlite:memory` kinds with a `VECTOR_EMBEDDING` function using compatible ONNX encoder models
  ...
  - Adds metadata-only `npx @cap-js/ai check-model <model>` to report likely model compatibility before downloading model artifacts; installation remains the definitive graph and runtime validation
  ...
  - Discovers compatible Hugging Face ONNX encoder layouts through the optional `@huggingface/hub` peer, recognizes common Transformers configuration aliases, and inspects the downloaded graph through the optional `onnx-proto` peer before installation
